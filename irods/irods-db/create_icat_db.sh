...
# PostgreSQL 15+ removed CREATE permission on public schema from PUBLIC by default
# Grant it to the irods user so iRODS can create its tables
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" -d "${ENV_IRODS_DB_NAME}" <<-EOSQL
GRANT CREATE ON SCHEMA public TO ${ENV_IRODS_DB_USERNAME};
EOSQL
